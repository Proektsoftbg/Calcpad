...
function activate(context) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider('calcpad', {
    ...
    const runCommand = vscode.commands.registerCommand('calcpad.run', async () => {
        ...
        const cliPath = config.get('cliPath', 'C:\\Program Files\\Calcpad\\Cli.exe');
        // Check if CLI exists
        ...
        vscode.window.withProgress({
            ...
            return new Promise((resolve, reject) => {
                (0, child_process_1.exec)(`"${cliPath}" "${filePath}" html -s`, (error, stdout, stderr) => {
                    if (error) {
                    ...
                    setTimeout(() => {
                        if (fs.existsSync(htmlPath)) {
                            ...
                            if (currentPanel) {
                                ...
                                // Create new panel
                                currentPanel = vscode.window.createWebviewPanel('calcpadOutput', `Calcpad: ${baseName}`, vscode.ViewColumn.Beside, {
                                    enableScripts: true,
                            ...
                            htmlContent = htmlContent.replace(/src="(?!http|data:)([^"]+)"/g, `src="${webviewUri}/$1"`);
                            htmlContent = htmlContent.replace(/href="(?!http|#)([^"]+)"/g, `href="${webviewUri}/$1"`);
    ...
    const openCommand = vscode.commands.registerCommand("calcpad.open", async () => {
        ...
        const config = vscode.workspace.getConfiguration('calcpad');
        const exePath = config.get('Path', 'C:\\Program Files\\Calcpad\\Cli.exe');
        (0, child_process_2.spawn)(exePath, [filePath], {
            detached: true,
            stdio: "ignore"
        }).unref();
    });
