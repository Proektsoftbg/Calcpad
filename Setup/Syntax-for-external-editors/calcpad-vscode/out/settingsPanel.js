...
exports.registerSettingsCommand = registerSettingsCommand;
...
function registerSettingsCommand(context) {
    ...
    function createPanel() {
        const panel = vscode.window.createWebviewPanel('calcpadSettings', 'Calcpad Settings', vscode.ViewColumn.One, { enableScripts: true });
    ...
    return vscode.commands.registerCommand("calcpad.settings", async () => {
        ...
        currentSettingsPath = config.get('settingsPath', 'C:\\Program Files\\Calcpad\\Settings.Xml');
...
function getSettingsHtml(s) {
    const checked = (val) => val === 'true' ? 'checked' : '';
    const selected = (current, option) => current === option ? 'selected' : '';
    const degSelected = (val, opt) => val === opt ? 'selected' : '';
